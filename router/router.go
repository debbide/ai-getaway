...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client) *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		admin := api.Group("/admin", middleware.Auth(cfg, db), middleware.AdminOnly())
		{
			...
			admin.GET("/billing-groups", adminController.BillingGroups)
			admin.POST("/billing-groups", adminController.CreateBillingGroup)
			admin.PUT("/billing-groups/:id", adminController.UpdateBillingGroup)
			admin.DELETE("/billing-groups/:id", adminController.DeleteBillingGroup)
			...
			admin.POST("/openai-oauth/auth-url", adminController.GenerateOpenAIOAuthURL)
			admin.POST("/openai-oauth/exchange-code", adminController.ExchangeOpenAIOAuthCode)
			admin.POST("/openai-oauth/refresh-token", adminController.RefreshOpenAIToken)
			admin.POST("/polling-pool-accounts/:id/openai-usage", adminController.RefreshOpenAIAccountUsage)
